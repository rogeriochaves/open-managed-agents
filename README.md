...
|---|:---:|---|
| `@open-managed-agents/server` | 195 | **Schema ↔ handler alignment lint** (regression guard for the silent-no-op class — every field declared on a zod Body/Query schema must be referenced by the handler body or listed as an intentional exception with a reason), **cursor pagination across every list route** (agents/sessions/environments/vaults — `after_id` + same-second tie tradeoff + unknown-id fallback), **date range filters** (`created_at[gt|gte|lt|lte]` on agents+sessions via shared helper), **global auth guard** (public-path allowlist, 401 on unauth, session cookie / Bearer / x-api-key unlock, bogus-token rejection), **agent builder chat** (real LLM round trip, fenced-draft parsing, 503 when no provider, done flag, prior-draft preservation), **MCP connector credential storage** (encrypted-at-rest tokens, upsert, connected flag, delete roundtrip, org-scoped isolation), **real MCP client + engine tool loop** (listTools via StreamableHTTPClientTransport with Bearer auth, `__mcp__` prefix routing, degraded-mode skip on broken connectors, 401/502 error shapes, cross-org isolation), **in-process MCP fixture round-trip**, **admin-created users with initial bcrypt password + login verification**, **cooperative session stop** (status=terminated + engine bails between iterations + transcript event), auth flow (login/logout/change-password + SSO discovery), agents (create/update/archive + metadata merge + versioning), sessions + events + archive + cascade delete + stop, providers CRUD (Anthropic/OpenAI/Google/Mistral/Groq/OpenAI-compatible/Ollama via Vercel AI SDK), environments + MCP discovery + delete + metadata patch merge, vaults + AES-256-GCM encryption round-trip + archive + delete + display_name update + metadata patch merge, usage & cost aggregation, governance IAC config loading, governance direct CRUD (orgs/teams/projects/members/policies/users/audit), Postgres SQL placeholder translator, automatic audit log writes on every mutation, team-scoped provider access enforcement (admin bypass + multi-team "any blocks"), team-scoped MCP policy enforcement, audit log `details` JSON parsing |
| `@open-managed-agents/web` | 165 | Sidebar with LangWatch-style sections, **two-column Quickstart** with real agent-builder chat (send/reply, draft preview, 503 error path), **agent detail with edit + archive** (name/description/system/model/mcp_servers editable, optimistic concurrency via version token), **vault detail full credential CRUD** (create modal, delete confirm, error-inside-modal on conflict), **environment detail** (unrestricted vs limited networking, allowed_hosts, packages grouped by manager), **session detail with tool_use↔tool_result pairing** (red Error badge on `is_error:true`, → tool-name prefix, execution duration, SSE streaming + Send composer + Stop button, download events JSON), **sessions list bulk archive** (multi-select wired end-to-end), **Settings → Audit log tab** (lazy fetch, resource-type filter, actor resolution), **Settings → Organization Add team + Add user modals with initial password**, **Settings → Governance interactive toggles + policy cycling + RPM/budget inputs**, **fetch-level api client contract test** (every mutation hits the correct URL/method — regression guard for `892989c`), **401→/login auth redirect** (QueryCache.onError hook, cross-origin credentials), **LoginPage full flow** (happy path, 401 error, in-flight state, native required guard), agents/sessions/environments/vaults/settings/usage list pages, MCP connector browser with connect modal |
| `@open-managed-agents/cli` | 24 | Client points at self-hosted server (not `api.anthropic.com`), API key precedence; **fetch-level contract tests** asserting every `oma agents/sessions/environments/vaults` command's underlying Anthropic-SDK call lands on the correct route + HTTP method (POST for update/archive, DELETE for hard delete) — regression guard for the same class of bug that `892989c` caught on the web side |
| `@open-managed-agents/scenario-tests` | 3 | End-to-end tests against the live server + real Anthropic provider + gpt-5-mini judge via LangWatch Scenario (opt-in with `OMA_SCENARIO_ENABLED=1`): simple factual question, multi-turn clarification dialogue, and the agent-builder chat iteratively refining a support-agent draft |
| **Total** | **387** | |
...
├── specs/              # BDD feature specifications (62 files)
...
pnpm test         # Run tests (387 tests across 43 suites)
