#!/usr/bin/env node
/**
 * Capture the Add environment modal on the live dev server.
 */
...
async function launchChrome() {
  const userDataDir = `/tmp/oma-env-qa-${Date.now()}`;
...
async function main() {
  ...
  try {
    ...
    await evaluate(cdp, `
      (() => {
        const btn = Array.from(document.querySelectorAll('button')).find(b => b.textContent?.trim() === 'Add environment');
        btn?.click();
        return btn ? 'ok' : 'not-found';
      })()
    `);
    await new Promise((r) => setTimeout(r, 600));
    await screenshot(cdp, join(OUT_DIR, "env-qa-add-modal.png"));
  } finally {
    try { cdp.close(); } catch {}
