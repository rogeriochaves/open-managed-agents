#!/usr/bin/env node
/**
 * Capture vault detail + Add credential modal.
 * Expects VAULT_ID env var.
 */
...
if (!VAULT_ID) { console.error("VAULT_ID required"); process.exit(1); }

async function launchChrome() {
  const userDataDir = `/tmp/oma-vault-qa-${Date.now()}`;
...
async function screenshot(cdp, path) {
  ...
  console.log("[vault-qa] saved", path);
}
...
async function main() {
  ...
  try {
    ...
    // Vault detail with one real credential
    await navigate(cdp, `http://localhost:5173/vaults/${VAULT_ID}`);
    await new Promise((r) => setTimeout(r, 1500));
    await screenshot(cdp, join(OUT_DIR, "vault-qa-01-detail.png"));

    // Open Add credential modal
    await evaluate(cdp, `
      (() => {
        const btn = Array.from(document.querySelectorAll('button')).find(b => b.textContent?.trim() === 'Add credential');
        btn?.click();
        return btn ? 'ok' : 'not-found';
      })()
    `);
    await new Promise((r) => setTimeout(r, 600));
    await screenshot(cdp, join(OUT_DIR, "vault-qa-02-add-modal.png"));
  } finally {
    try { cdp.close(); } catch {}
